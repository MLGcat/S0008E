    ...
    for(int s = 1; s <= samples; s++)
    {
        ...
        for(int y = 0; y < height; y++)
        {
            ...
            for(int x = start; x < width; x+=2)
            {
                ...
                image[(y * width + x) * 3] = (image[(y * width + x) * 3]*(s-1) + pixelColor[0]*255) * test;
                image[(y * width + x) * 3+1] = (image[(y * width + x) * 3+1]*(s-1) + pixelColor[1]*255) * test;
                image[(y * width + x) * 3+2] = (image[(y * width + x) * 3+2]*(s-1) + pixelColor[2]*255) * test;
                
                if(x > 1)
                {
                    image[(y * width + x) * 3-1] = (image[(y * width + x) * 3]  *(s-1) + (pixelColor[0] + prevPixelColor[0])*127.5) * test;
                    image[(y * width + x) * 3-2] = (image[(y * width + x) * 3+1]*(s-1) + (pixelColor[1] + prevPixelColor[0])*127.5) * test;
                    image[(y * width + x) * 3-3] = (image[(y * width + x) * 3+2]*(s-1) + (pixelColor[2] + prevPixelColor[0])*127.5) * test;
                }
