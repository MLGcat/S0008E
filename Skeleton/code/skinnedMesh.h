                ...
                for (unsigned int i = 0; i < 21; i++)
                {
                    if (header->vertexComponentMask & (1<<i))
                    {
                        ...
                        switch (1<<i)
                        {
                            ...
                            case N2ColorUB4N:     glVertexAttribPointer(bufferCount, 4, GL_UNSIGNED_BYTE, GL_FALSE, sizeof(GLfloat) * header->vertexWidth, (GLvoid*)offset);  test += 4; offset += sizeof(GLbyte) * 4;  break;
